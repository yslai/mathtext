...
namespace gluon {
	...
	public:
		...
		inline point_t(const mathtext::point_t &point)
		{
			_x[0] = point[0];
			_x[1] = point[1];
		}
		...
		inline operator mathtext::point_t(void) const
		{
			return mathtext::point_t(_x[0], _x[1]);
		}
	...
	public:
		...
		inline affine_transform_t(mathtext::affine_transform_t &t)
		{
		...
		operator std::string(void) const;
		inline operator mathtext::affine_transform_t(void) const
		{
			return mathtext::affine_transform_t(_a[0], _a[1], _a[2],
												_a[3], _a[4], _a[5]);
		}
	};
	...
	private:
		point_t _vertex[2];
	public:
		inline line_t(const point_t &vertex_0,
					  const point_t &vertex_1)
		{
			_vertex[0] = vertex_0;
			_vertex[1] = vertex_1;
		}
		inline const point_t *vertex(void) const
		{
			return _vertex;
		}
		inline point_t *vertex(void)
		{
			return _vertex;
		}
		inline point_t operator[](const int n) const
		{
			return _vertex[n];
		}
		inline point_t &operator[](const int n)
		{
			return _vertex[n];
		}
		inline point_t direction(void) const
		{
			return _vertex[1] - _vertex[0];
		}
		inline bool operator==(const line_t &line) const
		{
			return (_vertex[0] == line._vertex[0] &&
					_vertex[1] == line._vertex[1]);
		}
		inline bool operator!=(const line_t &line) const
		{
			return (_vertex[0] != line._vertex[0] ||
	...
	class polygon_t {
	private:
		std::vector<point_t> _vertex;
	public:
		inline polygon_t(void)
		{
			_vertex.clear();
		}
		inline const std::vector<point_t> vertex(void) const
		{
			return _vertex;
		}
		inline std::vector<point_t> &vertex(void)
		{
			return _vertex;
		}
		inline point_t operator[](const int n) const
		...
		inline void push_back(const point_t &vertex)
		{
	...
	public:
		...
		inline bounding_box_t(mathtext::bounding_box_t b)
			: _advance(b.advance()),
			  _italic_correction(b.italic_correction())
		{
			_lower_left[0] = b.lower_left()[0];
			_lower_left[1] = b.lower_left()[1];
			_upper_right[0] = b.upper_right()[0];
			_upper_right[1] = b.upper_right()[1];
		}
